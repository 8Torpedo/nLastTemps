...
#define I2C_ADDR_7BIT     (0x90) // temperature sensor
...
static void sendI2CMaster(uint16_t AddressI2C, bool ledStateOut, uint8_t regAddr, uint8_t messageByte)
{
	uint8_t SendData[10];
	I2C_PARAM_T param;
	I2C_RESULT_T result;
	...
	/* Setup I2C send for address/send, send desired LED state, then stop */
		...
		/* 7-bit address temperature sensor */
	SendData[index++] = (uint8_t) AddressI2C;
	SendData[index++] = regAddr;			/* I2C device regAddr */
	SendData[index++] = messageByte;	/* I2C device regVal */
...
int main(void)
{
	...
	const uint8_t TAB_SIZE = 10;
	const uint8_t LAST_TEMPS = 4; // How many measures must be checked with average
	uint8_t temp;
	uint8_t table[TAB_SIZE];
	uint8_t index = 0;
	uint8_t average = 0;
	while (1) {
		...
		case 0:
			...
			sendI2CMaster(I2C_ADDR_7BIT, ledState, 0x00, 0);
			break;
		...
		default:
			...
			readI2CMaster(I2C_ADDR_7BIT, &ledState, &temp);
			table[index]=temp;
			printf("Temp [%d] = %d\n", index, table[index]);
			index++;
			if (10==index){
				average = averageTemp(table, TAB_SIZE);
				printf("Average: %d\n", average);
				lastTemps(table, average, TAB_SIZE, LAST_TEMPS);
				index=0;
			}
			break;
		}
